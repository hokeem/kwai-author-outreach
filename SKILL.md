...
**Instagram DM：**
读取 `references/ig-dm-guide.md` 了解 IG DM 发送方式和 API 踩坑结论。**IG 对第三方 API 发 DM 限制严格，只能通过浏览器手动操作。**
...
- **IG DM 不可 API 自动化**：instagrapi sessionid 登录无法发 DM（`login_required`），密码登录触发 ChallengeRequired，requests+cookies 调 API 返回 200 但实际未发送。只能浏览器手动操作
